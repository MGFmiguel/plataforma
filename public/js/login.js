...
function renderMode() {
  ...
  subtitle.textContent = isRegistering
    ? "Seu espaço de cuidado começa com um primeiro passo."
    : "Acesse seu espaço de cuidado e conexão.";
  submitLabel.textContent = isRegistering ? "Começar agora" : "Entrar";
  switchLabel.textContent = isRegistering
    ? "Já tem uma conta?"
    : "Ainda não faz parte?";
  switchButton.textContent = isRegistering
    ? "Entrar no portal"
    : "Criar minha conta";
  document.querySelector("#password").autocomplete = isRegistering
    ? "new-password"
    : "current-password";
...
loadCsrfToken()
  .then(() => fetch("/api/auth/me", { credentials: "same-origin" }))
