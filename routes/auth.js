...
const { createToken, getCsrfToken, requireCsrf } = require("../middleware/csrf");
...
const registerLimit = createRateLimit({
  windowMs: 60 * 60 * 1000,
  max: 5,
  message: "Muitos cadastros a partir deste endereco. Tente novamente mais tarde.",
});
...
function validateRegistration(name, email, password) {
  ...
  if (normalizedName.length < 2 || normalizedName.length > 100) return { error: "Informe um nome entre 2 e 100 caracteres." };
  if (!validEmail || normalizedEmail.length > 254) return { error: "Informe um email valido." };
  if (normalizedPassword.length < 6 || normalizedPassword.length > 8) return { error: "A senha deve ter entre 6 e 8 caracteres." };
...
function createAuthenticatedSession(request, userId) {
  return new Promise((resolve, reject) => {
    request.session.regenerate((error) => {
      ...
      request.session.save((saveError) => (saveError ? reject(saveError) : resolve(request.session.csrfToken)));
    });
  });
}
...
router.post("/login", loginLimit, requireCsrf, async (request, response, next) => {
  try {
    ...
    response.json({ user: { id: user.id, name: user.name, email: user.email }, csrfToken });
  } catch (error) {
    next(error);
  }
});

router.post("/register", registerLimit, requireCsrf, async (request, response, next) => {
